  ...
  it("enforces policy, approvals, retries, rate limits, and company boundaries for connected remote MCP calls", async () => {
    ...
    try {
      ...
      expect(approvalInteraction).toMatchObject({
        ...
        payload: {
          version: 1,
          // The approval card names the connection the write goes through:
          // an approver deciding on a side-effecting call must be able to see
          // which of several connections to the same app it targets.
          prompt: "Approve KV Set (Shopify Store)?",
