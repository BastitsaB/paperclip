...
import { workspaceGitOperationScheduler } from "./worktree-git-operation-scheduler.js";
...
} from "./worktree-runtime-read-model.js";
...
async function quarantineRestoreDirtyWorkspaceBranch(input: {
  ...
  const { ensureGitWorktreeBranchCoherent } = await import("./worktree-runtime.js");
  try {
  ...
  return {
    ...
    getById: async (id: string) => {
      const row = await db
        ...
        .then((rows) => rows[0] ?? null);
      if (!row) return null;
      const { refreshPersistedRuntimeServiceHealth } = await import("./worktree-runtime.js");
