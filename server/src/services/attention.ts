 ...
 * readRunIssueId (issueId ?? taskId): a run is on issue X when
 * context_snapshot.issueId is X, or when issueId is absent and taskId is X; a
 * run has no issue context when issueId is empty, or when issueId is absent and
 * taskId is absent or empty. Each EXISTS is a plain conjunction with created_at
    ...
    const rows = (await db.execute(sql`
      ...
      WHERE (
        ...
        WHERE r.company_id = ${companyId}
          AND r.agent_id = k.agent_id
          AND r.created_at > k.created_at
          -- Mirrors readRunIssueId: issueId ?? taskId, so an empty issueId
          -- makes the run context-less regardless of taskId.
          AND (
            (r.context_snapshot ->> 'issueId') = ''
            OR (
              (r.context_snapshot ->> 'issueId') IS NULL
