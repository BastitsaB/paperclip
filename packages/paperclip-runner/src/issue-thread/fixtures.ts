...
export const CAPABILITY_UI_SHOT_SLUGS = [
  ...
  "cm-provider-labels",
  "ac-streaming",
  "ac-session-continuity",
  "ac-tool-read",
  "ac-tool-mutation",
  "ac-completion-control",
  "ac-tool-denied",
  "ac-tool-duplicate",
  "ac-eventstream-recovery",
  "ac-memory-reconciliation",
  "ac-interrupt",
  "ac-warm-restore",
  "ac-limits",
  "ac-session-delete",
...
const PROVIDER_SCENARIOS: Record<string, { family: ProviderFixtureFamily; eventType: string; title: string; payload: Record<string, CapabilityJsonValue> }> = {
  ...
  "ac-streaming": { family: "model_identity", eventType: "agentcore.message.reconciled", title: "AgentCore streaming", payload: { provider: "aws_agentcore", driver: "aws_agentcore_harness_api", executionKind: "remote_service", runtimeSessionId: "paperclip-4a7e-session", eventStreamFrames: 14, preview: "Streaming response…", authoritative: "The complete AgentCore response.", summary: "AWS EventStream text was normalized into provisional deltas and one authoritative completed item." } },
  "ac-session-continuity": { family: "context", eventType: "agentcore.session.continued", title: "AgentCore session continuity", payload: { runtimeSessionId: "paperclip-4a7e-session", paperclipRuns: ["run-aws-1", "run-aws-2"], runnerPid: 48121, providerPid: null, warmTurnMs: 744, summary: "Two isolated Paperclip run attachments reused one AWS runtime session and its Memory history." } },
  "ac-tool-read": { family: "tool_execution", eventType: "semantic_tool.result", title: "Authorized AgentCore read", payload: { schema: "paperclip.semantic_tool.result.v1", callId: "aws-tool-read-1", operationId: "get_task_context", status: "completed", duplicate: false, result: { identifier: "MCK-31", stateRevision: 4 }, allowedToolsCount: 8, summary: "runnerd validated and executed an exact-catalog read; AWS received only its inline result." } },
  "ac-tool-mutation": { family: "tool_execution", eventType: "semantic_tool.result", title: "Authorized AgentCore mutation", payload: { schema: "paperclip.semantic_tool.result.v1", callId: "aws-tool-write-1", operationId: "report_progress", status: "completed", executionCount: 1, result: { stateRevision: 5 }, summary: "The durable Paperclip result preceded the AgentCore continuation and the mutation ran exactly once." } },
  "ac-completion-control": { family: "review", eventType: "agentcore.completion.controlled", title: "AgentCore finish, block, and yield", payload: { accepted: "finish_task", alternatives: ["block_task", "yield_turn"], endTurnAloneAccepted: false, freeChatEndTurnAccepted: true, summary: "Governed runs require a structured disposition while Runner Lab free chat may end normally." } },
  "ac-tool-denied": { family: "safety", eventType: "semantic_tool.denied", title: "AgentCore tool rejected", payload: { operationId: "shell", allowedTools: ["pc_get_task_context_a73f0a"], reasons: ["not in attached catalog", "schema-invalid input"], paperclipExecuted: false, status: "failed", summary: "Unknown or malformed inline functions fail closed before control-plane execution." } },
  "ac-tool-duplicate": { family: "tool_execution", eventType: "semantic_tool.delivery.reconciled", title: "AgentCore duplicate-safe delivery", payload: { toolUseId: "tooluse-aws-44", executionCount: 1, deliveryAttempts: 2, assistantToolUseRestored: true, userToolResultRestored: true, status: "completed", summary: "A durable result was reused and the governed mutation was never repeated." } },
  "ac-eventstream-recovery": { family: "context", eventType: "agentcore.eventstream.recovered", title: "EventStream recovery", payload: { invocationId: "invoke-aws-9", chunkedFrames: 11, replayedFrames: 3, duplicatesIgnored: 2, status: "completed", summary: "Chunked AWS EventStream output resumed without duplicating normalized PRP items." } },
  "ac-memory-reconciliation": { family: "context", eventType: "agentcore.memory.reconciled", title: "Memory reconciliation", payload: { toolUseId: "tooluse-aws-44", snapshotEventId: "mem-before-208", stopRuntimeRequested: true, committedAfterAmbiguity: true, replayedMutation: false, status: "completed", summary: "A response-loss ambiguity was checked against AgentCore Memory before the runner advanced." } },
  "ac-interrupt": { family: "provider_notice", eventType: "agentcore.turn.interrupted", title: "AgentCore turn interrupted", payload: { stopRuntimeSession: true, phases: ["generation", "waiting_for_tool"], status: "interrupted", recoverable: true, runtimeSessionIdRetained: true, summary: "Interrupt stopped AWS compute while retaining the resumable Memory-backed session." } },
  "ac-warm-restore": { family: "context", eventType: "agentcore.session.restored", title: "AgentCore warm suspension and cold restore", payload: { runtimeSessionId: "paperclip-4a7e-session", warmRunnerReused: true, runtimeStopped: true, runnerRestarted: true, memoryReused: true, status: "completed", summary: "Warm and per-turn lifecycles preserve one AgentCore identity across Runtime restarts." } },
  "ac-limits": { family: "provider_notice", eventType: "agentcore.limit.reached", title: "AgentCore invocation limits", payload: { currency: "USD", costSource: "paperclip_estimate", estimatedCost: 0.17, ceiling: 1, maxIterations: 8, maxOutputTokens: 4096, timeoutSeconds: 300, status: "completed", recoverable: true, summary: "Token, iteration, and time bounds are hard; the visible dollar ceiling is explicitly an estimate." } },
  "ac-session-delete": { family: "provider_notice", eventType: "agentcore.session.deleted", title: "Delete AWS session", payload: { stopRuntimeSession: true, memoryEventsDeleted: 12, paginationComplete: true, localPointerRemovedAfterRemoteSuccess: true, status: "completed", summary: "Confirmed deletion stopped compute and purged every matching short-term Memory event." } },
  "ac-iam-profile": { family: "provider_notice", eventType: "agentcore.profile.qualified", title: "AgentCore IAM and profile drift", payload: { harnessVersion: "3", endpointQualifier: "paperclip", invocationRoleAssumed: true, runtimeCommandDenied: true, retentionDays: 90, driftDetected: false, status: "completed", summary: "The probe verified the pinned Harness, endpoint, Memory, model, and least-privilege invocation role." } },
  "ac-redacted-failure": { family: "provider_notice", eventType: "agentcore.failure.redacted", title: "Redacted AWS failure", payload: { severity: "error", category: "authentication", recoverable: true, userActionable: true, redactedFields: ["authorization", "x-amz-security-token", "x-amz-signature", "access key"], retainedSignedRequest: false, providerRuntime: "AWS cloud", providerPid: null, forbiddenLabelsAbsent: ["Real Codex", "Codex is thinking"], status: "failed", summary: "AWS credentials, signed headers, and raw remote errors never entered durable evidence." } },
};
