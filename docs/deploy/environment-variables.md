...
| `PAPERCLIP_AGENT_PROCESS_NICE` | (unset) | Unix nice value (integer `0`–`19`) applied to locally launched agent processes (local adapters, ACPX agents, native runner, workspace runtime services); everything they start inherits it. Keeps the API server responsive when agent builds saturate the CPUs of a shared host or container. Unset, empty, or `0` leaves priority untouched; an invalid value is logged once and ignored. The value is absolute, so it has no effect if the server already runs at a higher nice value. No effect on Windows or remote/sandbox targets. `10` is a reasonable choice for single-container deployments. |
